...
namespace Utilize.Report
{
    public partial class UCFiltering : UserControl
    {
        ...
        public IEnumerable<object> GetCheckedObjects()
        {   
           return cmbSource.CheckedItems.Select(item => item.DataValue);
        }

        /// <summary>
        /// مقادیر به صورت key1,key2,...
        /// </summary>
        public string GetSelectedItems()
        {
            ...
            if (chbTitle.Checked)
            {
                ...
                filter = items.Aggregate(filter, (current, item) => current + (current.Trim().Length == 0 ? item.DataValue.ToString().Trim() : " , " + item.DataValue.ToString().Trim()));
            }
            return filter;
        }
        /// <summary>
        ...
        {
            var filter = "";
            var items = cmbSource.Items;

            foreach (var item in items)
            {
                filter += filter.Trim().Length == 0
                    ? item.DataValue.ToString().Trim()
                                            : " , " + item.DataValue.ToString().Trim();
            }
            return filter;
        }

        public IEnumerable<Entity> GetAllItems<Entity>() where Entity : class
        {
            return cmbSource.DataSource as IEnumerable<Entity>;
        }
        ...
        private void cmbSource_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
                cmbSource.SelectedText = cmbSource.Contains(e.KeyChar.ToString());
        }
    }
}
