...
{
    public partial class frmStressType : BaseForm
    {
        #region VAR
        int templateID = 0;
        ...
        public frmStressType(string Model_Type)
        {
            InitializeComponent();
        ...
        public frmStressType(int tID, int Type, int N, DateTime fromDate, DateTime toDate, string name, float value, int FType, string MType)
        {
            InitializeComponent();
            templateID = tID;
            _model_Type = MType;
        ...
        {
            if (templateID == 0)
                new DAL.SwartDataSetTableAdapters.MR_Stress_Model_NewSelectCommandTableAdapter().MR_Stress_Model_NewInsertCommand(txtTemplateName.Text, ChangeType.ToString(), N, DateFrom, DateTo, A, Formula,_model_Type);
            else
            {
